<script lang="ts">
  ...
  import { SprayFilter } from "$lib/filters/spray/command"
  ...
  const spray = new SprayFilter({ modeIdx: 4 })

  const filterStack = new FilterStack({ blurX, blurY, contrast, spray })
  ...
  const sketch: SketchFn = ({ gl, canvas }) => {
    ...
    return {
      ...
      drawOnFrame() {
        ...
        stackRenderer.beginPath()
        uniforms && spray.applyUniforms(uniforms)
        plane.draw({ primitive: "TRIANGLES" })
        stackRenderer.endPath()
...
<div>
  スプレーの強さ<Slider bind:value={spray.uSprayMixRatio} min={0.5} max={1} step={0.01} />
</div>
<div>
  スプレーの広がり<Slider bind:value={spray.uSpraySpread} min={3} max={100} step={1} />
</div>
